...
@eagle.tests.common.post_install(False)
class LebupayCommon(PaymentAcquirerCommon):

    def setUp(self):
        super(LebupayCommon, self).setUp()
...
@eagle.tests.common.at_install(False)
@eagle.tests.common.post_install(False)
class LebupayForm(LebupayCommon):

    def test_10_Lebupay_form_render(self):
        base_url = self.env['ir.config_parameter'].get_param('web.base.url')
        # be sure not to do stupid things
        self.assertEqual(self.lebupay.environment, 'test', 'test without test environment')
