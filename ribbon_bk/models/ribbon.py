...
class ribbonMedalForce(models.Model):
    _name="ribbon_bk.force"
    _description = "Name of the Force ie; Police, Army ..."
    name=fields.Char("Force Name")
    image=fields.Binary("Logo")
    description=fields.Char("Description")
    ribbon_ids=fields.Many2many("ribbon_bk",'ribbon_force_medal_rel',column1='force_id',column2="ribbon_id",string="ribbons")
    ranks=fields.One2many("ribbon_bk.rank",'force_id')
...
class RibbonMedalRibbonProduct(models.Model):
    _name="ribbon_bk.ribbon_bk.product"
...
class ribbonAcquisitionRule(models.Model):
    _name="ribbon_bk.acquisition.rule"
...
class ribbonRegulation(models.Model):
    ...
    force_id=fields.Many2one('ribbon_bk.force',"Force Name")
    serial=fields.Integer("Serial")
    acquisition=fields.Many2one("ribbon_bk.acquisition.rule","Acquisition Rule")
...
class ribbonMedalForceUnit(models.Model):
    _name = "ribbon_bk.force.unit"
    _description = "Name of Units of a Force"
    name = fields.Char("Unit",translate=True)
    force_name=fields.Many2one("ribbon_bk.force","Force")
    address=fields.Char("address",translate=True)
    parent_unit=fields.Many2one("ribbon_bk.force.unit","Parent Unit",translate=True)
    chief_rank=fields.Many2one("ribbon_bk.rank","Rank of Chief",translate=True)
    chief=fields.Many2one("ribbon_bk.post","Chief",translate=True)

class RibbonMedalsPost(models.Model):
    _name="ribbon_bk.post"
