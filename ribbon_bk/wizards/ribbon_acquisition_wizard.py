...
class RibbonMedalAcquisitionWizard(models.TransientModel):
    _name ='ribbon_bk.acquisition.wizard'
    _description='Get acquired ribbons and medals by a force Member'
    ribbon_holder=fields.Many2one('res.partner',"Person")
    force_id=fields.Many2one("ribbon_bk.force","Force Name",related="ribbon_holder.force_id")
    id_no=fields.Char("ID No",related="ribbon_holder.id_no")
    rank=fields.Many2one("ribbon_bk.rank","Rank",related="ribbon_holder.rank")
    unit=fields.Many2one("ribbon_bk.force.unit","Unit",related="ribbon_holder.unit")
    post=fields.Many2one("ribbon_bk.post","Post",related="ribbon_holder.post")
    ...
    services=fields.Many2one("ribbon_bk.personal.service")
    awards=fields.Many2many("ribbon_bk.personal.award")
    missions=fields.Many2many("ribbon_bk.personal.mission")
    acquired_ribbons=fields.Many2many("ribbon_bk.acquired.ribbon_bk.wizard","party_acquired_rel")
    ...
    def get_acquired_ribbons(self):
        if self.ribbon_holder.retired:
            ribbon_for_age=self.env["ribbon_bk.regulation"].search([('force_id', '=', self.force_id.id),
        ...
        for rec in ribbon_for_age:
            input=self.env["ribbon_bk.acquired.ribbon_bk.wizard"].create({
        ...
        ribbon_for_awards=self.env["ribbon_bk.personal.award"].search([("partner_id","=",self.ribbon_holder.id)])
        ...
        ribbon_for_missions = self.env["ribbon_bk.personal.mission"].search(
    ...
    def getRibonImage(self):
        ...
        for rec in self.acquired_ribbons:
            ...
            image_set = image_set + '<img style=" position:absolute;  left: '+str(position_left)+point_unit+';   width:'+str(point_length)+point_unit+';'\
                        + 'height:'+ str(point_height)+point_unit+';  border: 1px solid blue;z-index: 1;" src="../web/image/ribbon_bk.extension/'+str(rec.extension.id)+'/image"/>'+chr(10)
...
class ribbonMedalAcquiredRibbonWizard(models.TransientModel):
    _name="ribbon_bk.acquired.ribbon_bk.wizard"
...
class ribbonMedalquotation(models.TransientModel):
    ...
    ribbon_details=fields.One2many("ribbon_bk.ribbon_bk.quotation.details",'quotation_id')
    ...
    bmedal_details=fields.One2many("ribbon_bk.ribbon_bk.quotation.details",'quotation_id')
    ...
    smedal_details = fields.One2many("ribbon_bk.ribbon_bk.quotation.details", 'quotation_id')
...
class  ribbonMedalribbonQuotationDetails(models.TransientModel):
    _name= "ribbon_bk.ribbon_bk.quotation.details"
    quotation_id=fields.Many2one('ribbon_bk.quotation')
...
class  ribbonMedalTunicMedalQuotationDetails(models.TransientModel):
    _name= "ribbon_bk.bmedal.quotation.details"
    quotation_id=fields.Many2one('ribbon_bk.quotation')
    product_id=fields.Many2one("product.product")
...
class  ribbonMedalMeskitMedalQuotationDetails(models.TransientModel):
    _name= "ribbon_bk.smedal.quotation.details"
    quotation_id=fields.Many2one('ribbon_bk.quotation')
    product_id=fields.Many2one("product.product")
